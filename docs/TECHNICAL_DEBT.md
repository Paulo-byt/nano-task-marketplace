...
- **Cancel-vs-approve write-skew window.** `cancelTask` and `approveApplication` (see [Resolved → Applications](#applications-1)) each check the other's table before committing (Fix #11), but under Postgres's default READ COMMITTED isolation this only narrows, and does not eliminate, the race between cancelling a task and approving one of its applications. Measured directly: a 25-iteration back-to-back concurrency stress test with no natural spacing between the two operations reproduced the bad state — `task=cancelled`, `application=approved`, `payout=pending` — in 24 out of 25 runs (~96%). This is a materially larger residual than the cancel-vs-retry write-skew window documented above (~4%): `markPayoutRetrying` is a single `UPDATE` statement, so its guard passing and the resulting state becoming visible to other transactions happen in the same commit, while `approveApplication` and `cancelTask` are each multi-statement transactions (`UPDATE`+`INSERT` and `UPDATE`+`UPDATE` respectively), leaving a much larger mutual blind spot for close to the whole duration of both transactions. Real usage's actual network round-trip latency between two separate HTTP requests narrows the practical window somewhat, but this residual should not be assumed as rare as the cancel-vs-retry case. Deferred because closing it fully needs the same larger mechanism already ruled out for cancel-vs-retry — explicit `SELECT ... FOR UPDATE` row locking (with carefully consistent lock ordering across both functions to avoid deadlock) or `SERIALIZABLE` isolation with retry-on-conflict logic — neither pattern is used anywhere else in this codebase today, and this is Arc Testnet-only with no production traffic. See `approveApplication`'s own doc comment in `services/applications/applicationsService.ts` for the full detail.
...
- **`approveApplication` had no atomic awareness of the parent task's funding status.** `approve/route.ts` checked `task.fundingStatus !== "funded"` only as a plain prior read, before ever calling `approveApplication` — not part of that write's own atomic condition, so a task cancellation could in principle race a concurrent approval and let a payout be created for an application whose task had just been (or was concurrently being) cancelled. **Resolved (Fix #11):** `approveApplication`'s conditional `UPDATE` (`services/applications/applicationsService.ts`) now includes an `EXISTS` clause requiring the parent task's live `fundingStatus` to still be `'funded'`, the same guard shape as `markPayoutRetrying` (Fix #9). This fully closes the *sequential* case — verified directly: approval against an already-cancelled, unfunded, or `'released'` task is unconditionally rejected with the existing `ApplicationNotApprovableError`, and normal approval and duplicate-payout protection are both unaffected. **It does not close the concurrent case.** A true simultaneous race between `cancelTask` and `approveApplication` was measured at a 96% (24/25) reproduction rate for the bad state (`task=cancelled`, `application=approved`, `payout=pending`) — far higher than the ~4% Fix #9 achieved for the structurally similar cancel-vs-retry pair, because both `approveApplication` and `cancelTask` are multi-statement transactions, so each side's own guard passes long before the other side's change is actually committed and visible. Deliberately left as documented residual risk rather than fixed further — see the [Low Priority security item](#security-1) below for the full detail.
